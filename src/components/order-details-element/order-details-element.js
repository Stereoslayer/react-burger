...
function OrderDetailsElement({data}) {
    const {ingredients} = useSelector((state) => state.ingredients);
    const navigate = useNavigate();
    const location = useLocation();

    const formattedIngredients = data.ingredients.map((item) => ingredients.find((el) => el._id === item));
    ...
    const getIngredientOverflow = () => {
        ...
        if (formattedIngredients.length > 6) {
            ...
            return (
                <li className={orderDetailsElementStyle.list_item + ' ' + orderDetailsElementStyle.last_image_list_item}
                    ...
                    <div className={orderDetailsElementStyle.last_image_box}>
                        <img className={orderDetailsElementStyle.image} alt={ingOverflow.name} src={ingOverflow.image}/>
            ...
            return (<li className={orderDetailsElementStyle.list_item + ' ' + orderDetailsElementStyle.ing_sixth}><img
                className={orderDetailsElementStyle.image} alt={ingOverflow.name} src={ingOverflow.image}/></li>)
        }
    }
